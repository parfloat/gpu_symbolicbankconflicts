...
static void dumpBankConflictCap2x(Executor &executor, ExecutionState &state,
	...
	std::cout << std::endl;
	//ma1.dump(executor, state, bcCond); //FIXME adrho uncomment these
	//ma2.dump(executor, state, bcCond); //FIXME adrho uncomment these
...
//adrianh
void HierAddressSpace::constructPerLineAccesses(Executor &executor, ExecutionState &state,
		std::vector<CorrespondTid> &cTidSets,
		unsigned capability,
		unsigned kernelNum, unsigned BINum){

	assert((capability > 1) && ("Cannot handle capability lower than 2.x -- for now"));

	for (auto memory : sharedMemories)
	{
		//	    GKLEE_INFO << "\n********** Start building warp accesses at SharedMemory "
		//	               << i++ << " **********\n";
		//	    if (GPUConfig::verbose > 0)
		//	      ii->dump(true);

		//read set
		MemoryAccessVec tmpRWSet = memory.readSet;

		while (!tmpRWSet.empty()) {
			MemoryAccessVec lineRWSet;

			AddressSpaceUtil::constructTmpRWSet(executor, state, tmpRWSet, lineRWSet, cTidSets,
					instAccessSets, divRegionSets, sameInstVecSets,
					GPUConfig::warpsize);

			lineReadSharedLines[kernelNum][BINum][&memory].push_back(lineRWSet);
		}


		//write set
		tmpRWSet = memory.writeSet;
		...
		while (!tmpRWSet.empty()) {
			...
			lineWriteSharedLines[kernelNum][BINum][&memory].push_back(lineRWSet);
		}
	}
