...
void Executor::encounterBarrier(ExecutionState &state,
  ...
  if (allThreadsBarrier) {
    ...
    //adrianh construct per line access in the address space
    state.addressSpace.constructPerLineAccesses(*this, state, state.cTidSets, DevCap, state.getKernelNum(), BINum);
     //adrianh end
