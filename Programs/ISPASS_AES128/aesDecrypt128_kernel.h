...
__global__ void aesDecrypt128( unsigned * result, unsigned * inData, int inputSize, unsigned *inKey)
{
    ...
    __shared__ UByte4 invSBoxBlock[256];
	...
	for (unsigned cnt=0; cnt<elemPerThread; cnt++) {
		TBOXE0(tx*elemPerThread + cnt).uival	= TBoxi0[tx*elemPerThread + cnt];
		TBOXE1(tx*elemPerThread + cnt).uival	= TBoxi1[tx*elemPerThread + cnt];
		TBOXE2(tx*elemPerThread + cnt).uival	= TBoxi2[tx*elemPerThread + cnt];
		TBOXE3(tx*elemPerThread + cnt).uival	= TBoxi3[tx*elemPerThread + cnt];
                INVSBOX(tx*elemPerThread + cnt).ubval[0]= inv_SBox[tx*elemPerThread + cnt];
	}
